...
@section('title', 'Tambah Pengawasan')
...
<div class="px-margin-desktop pt-8 pb-6 flex flex-col gap-4">
    <nav class="flex items-center text-on-surface-variant text-label-sm font-label-sm gap-2">
        ...
        <span class="material-symbols-outlined text-[16px]">chevron_right</span>
        <span class="text-primary font-bold">Tambah Pengawasan</span>
    </nav>
    <div class="flex justify-between items-end flex-wrap gap-4">
        <div>
            <h1 class="text-headline-lg font-headline-lg text-primary">Tambah Surat Tugas Pengawasan</h1>
            <p class="text-body-md font-body-md text-on-surface-variant mt-1">Buat surat tugas baru untuk kegiatan pengawasan, monitoring, atau reviu.</p>
        </div>
...
<div class="px-margin-desktop pb-margin-desktop">
    ...
    <div class="bg-surface-card border border-border-subtle rounded-xl shadow-sm overflow-hidden w-full max-w-5xl">
        <form action="{{ route('pengawasan.store') }}" method="POST" id="pengawasanForm">
            @csrf
                ...
                <div class="flex flex-col gap-2 md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <option value="Audit" {{ old('jenis_penugasan') == 'Audit' ? 'selected' : '' }}>Audit</option>
                            <option value="Reviu" {{ old('jenis_penugasan') == 'Reviu' ? 'selected' : '' }}>Reviu</option>
                            <option value="Monitoring" {{ old('jenis_penugasan') == 'Monitoring' ? 'selected' : '' }}>Monitoring</option>
                            <option value="Evaluasi" {{ old('jenis_penugasan') == 'Evaluasi' ? 'selected' : '' }}>Evaluasi</option>
                            <option value="Pendampingan" {{ old('jenis_penugasan') == 'Pendampingan' ? 'selected' : '' }}>Pendampingan</option>
                            <option value="Perjalanan Dinas Luar Daerah" {{ old('jenis_penugasan') == 'Perjalanan Dinas Luar Daerah' ? 'selected' : '' }}>Perjalanan Dinas Luar Daerah</option>
                        </select>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <option value="darat" {{ old('alat_angkut') == 'darat' ? 'selected' : '' }}>Transportasi Darat</option>
                            <option value="udara" {{ old('alat_angkut') == 'udara' ? 'selected' : '' }}>Transportasi Udara</option>
                        </select>
                        <span class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-outline-variant text-[18px]">expand_more</span>
                    </div>
                    @error('alat_angkut')
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div id="dasar-hukum-container" class="flex flex-col gap-2">
                            @if(old('dasar_hukum'))
                                @foreach(old('dasar_hukum') as $index => $dasar)
                                    <div class="dasar-hukum-item flex gap-2 items-start relative group">
                                        <textarea
                                            name="dasar_hukum[]"
                                            class="flex-1 p-3 rounded-lg border {{ $errors->has('dasar_hukum.'.$index) ? 'border-error focus:ring-error' : 'border-outline-variant focus:ring-primary' }} text-body-sm bg-surface-container-lowest focus:outline-none focus:ring-1 transition-colors min-h-[60px]"
            ...
            <div class="p-6 border-b border-border-subtle bg-white">
                ...
                <input type="hidden" id="penanggung_jawab_id" name="penanggung_jawab_id" value="{{ old('penanggung_jawab_id') }}" required>
                <input type="hidden" id="pengendali_teknis_id" name="pengendali_teknis_id" value="{{ old('pengendali_teknis_id') }}">
                <input type="hidden" id="ketua_tim_id" name="ketua_tim_id" value="{{ old('ketua_tim_id') }}" required>
                <div id="anggotaHiddenInputs">
                    @if(old('anggota'))
                        @foreach(old('anggota') as $anggotaId)
                            <input type="hidden" name="anggota[]" value="{{ $anggotaId }}">
                        @endforeach
                    @endif
                </div>
            ...
            <div class="p-6 bg-white">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex items-center gap-4">
                            <label class="flex items-center gap-2 cursor-pointer font-medium text-[13px] text-on-surface hover:text-primary transition-colors">
                                <input type="radio" name="penandatangan_type" value="definitif"
                                    ...
                                    {{ old('penandatangan_type', 'definitif') == 'definitif' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center gap-2 cursor-pointer font-medium text-[13px] text-on-surface hover:text-primary transition-colors">
                                <input type="radio" name="penandatangan_type" value="plh"
                                    ...
                                    {{ old('penandatangan_type') == 'plh' ? 'checked' : '' }}
                                    onchange="togglePenandatangan()"
                                    class="w-4 h-4 text-primary focus:ring-primary border-outline-variant">
                    ...
                    <div class="flex flex-col gap-2" id="penandatangan_definitif_info" style="{{ old('penandatangan_type', 'definitif') == 'definitif' ? '' : 'display:none;' }}">
                ...
                <div id="penandatangan_plh_fields" class="mt-4" style="{{ old('penandatangan_type') == 'plh' ? '' : 'display:none;' }}">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 rounded-xl border border-border-subtle bg-surface-container-lowest">
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="hidden" id="penandatangan_plh_nama" name="penandatangan_plh_nama" value="{{ old('penandatangan_plh_nama') }}">
                            <div class="relative">
                                <input
                                    type="text"
                                    ...
                                    value="{{ old('penandatangan_plh_nama') }}"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                value="{{ old('penandatangan_plh_jabatan') }}"
